...
- 左/右环 `IN` 的第 0 行上锚点分别为 `65/122`，比旧值更靠近图像中线。
...
速度值的单位是 15 ms 窗口内的编码器脉冲数，不是 RPM。目标速度按舵机转角平方降低，中小弯比原线性映射保留更高速度，大转角最低降到 `min_speed`。左右轮再按舵机转向加入最大 8 脉冲的差速偏置，最终单轮目标限制在 165–`max_speed`。电机 PI 输出经过最低有效 PWM 和上下行变化率限制后，再同步回 `pid.out`，避免隐藏积累。
...
- WiFi 调参第 6 通道为最低速度，第 7 通道为最大速度；两者范围为 165–220，并自动维持 `min_speed <= max_speed`，默认分别为 172/180。
- 调试助手示波器通道 0–7 依次为：中线误差、舵机误差、左右编码器实际速度、左右轮目标速度、左右轮实际应用 PWM。
