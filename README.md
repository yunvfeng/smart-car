...
本工程基于逐飞 STC32G144K 库和 Keil C251 工具链，实现摄像头循迹、左右环岛、编码器测速、电机与舵机闭环、IMU 横摆阻尼、靶点识别、激光时序和 WiFi 调试。
...
IN/OUT 阶段使用固定上锚点补线，避免断边直接进入舵机控制：
...
- 左/右环 `IN` 的第 0 行上锚点分别为 `57/130`，由 `65/122` 各向外移动 8 像素，并保持 `130 = 187 - 57` 的镜像关系。
- 左/右环 `OUT` 的第 0 行上锚点分别为 `30/157`，替代边缘列 `0/187` 的大偏置补线。
- 环内累计 40 帧后进入 `OUT`。
...
- 校零后的 Z 轴使用 `filtered = (3 * filtered + corrected) / 4` 低通滤波。
- 舵机保持 `Kp=4500`、`Kd=200`、`Kf=200`，再按 `out -= KG * gyro_z / 1024` 加入横摆阻尼；该修正单独限制在 -40～40 duty。
- 首次确认符号时保持车轮离地且 KG=0，手动将车头向右旋转：示波器通道 4 为正则设置 KG=+8，为负则设置 KG=-8。确认舵机向旋转反方向修正后再落地测试。
...
| `MOTOR_MIN_EFFECTIVE_PWM` | 1500 | 正向运行时的最低有效 PWM |
| `MOTOR_PWM_RISE_STEP` | 250 | 每 15 ms 最大 PWM 上升量 |
| `MOTOR_PWM_FALL_STEP` | 400 | 每 15 ms 最大 PWM 下降量 |
| `MAX_SPEED` | 180 | 默认最大目标速度，可通过 WiFi 调参 |
| `MAX_SPEED_TUNE_MAX` | 220 | 最大速度的在线调参上限 |
...
`MOTOR_MIN_EFFECTIVE_PWM=1500` 是运行中的硬下限：只要目标速度为正，实际 PWM 不会主动降到 0。若 PWM 1500 对应的机械速度已经高于 WiFi 设定目标，继续降低目标值不会让实车进一步降速。
...
- WiFi 第 3 通道为带符号 `GYRO_KG`，范围 -32～32；原曲率前馈 KF 固定为 200，不再由 WiFi 修改。第 6 通道为最低速度，第 7 通道为最大速度；两者范围为 165–220，并自动维持 `min_speed <= max_speed`，默认分别为 172/180。
- WiFi 通道 1～8 依次为：舵机 KP、舵机 KD、陀螺仪 KG、电机 KP、电机 KI、最低速度、最大速度、相机曝光。
- 调试助手示波器通道 0～7 依次为：中线误差、舵机误差、左右编码器实际速度、校零滤波后的 Z 轴、实际陀螺仪舵机修正量、左右实际 PWM。
- 调试图像只叠加 `R:环岛状态` 文本；不再绘制速度、参考列、IMU 或 KG 文字，减少程序占用。靶点十字标记仍保留。
...
2026-07-15 的最近一次 Keil C251 构建结果：
